...
function StudentViewCourseProgressPage(){
    ...
    async function fetchCurrentCourseProgress(){
        ...
        if(response?.success){
            if(!response?.data?.isPurchased){
                console.log(response?.data?.isPurchased, 'isPurchased');
                setLockCourse(true);
            }else{
                ...
                if(response?.data?.completed){
                    console.log(response?.data?.completed, 'isCompleted');
    ...
    return <div className="flex flex-col h-screen bg-[#1c1d1f] text-white">
        ...
        <div className="flex items-center justify-between p-4 bg-[#1c1d1f] border-b border-gray-700">
            ...
            <Button onClick={() => setIsSideBarOpen(!isSideBarOpen)} className="relative z-10">
